...
def _is_probability_column(column: object) -> bool:
    """Return True if a column name looks like an hourly probability series."""
...
def _convert_linefailprob_sheet(df: pd.DataFrame) -> pd.DataFrame:
    """Convert cumulative failure probabilities into incremental per-hour hazards.

    Some impact exports provide cumulative failure probabilities (monotonically
    increasing across the 48 hours). The Monte Carlo simulator, however, expects
    per-hour conditional probabilities (hazards). This helper detects rows that
    are non-decreasing and converts them into incremental probabilities while
    leaving already-independent rows untouched.
    """

    prob_cols = [col for col in df.columns if _is_probability_column(col)]
    if not prob_cols:
        return df
    ...
    # Identify rows that are approximately non-decreasing -> treated as cumulative.
...
def export_cluster_results(
    ...
    linefail_path: Optional[Path] = None,
) -> Path:
    """Persist clustering assignments and summaries to a companion workbook.
    
    Args:
        linefail_path: Optional path to the original linefailprob.xlsx file
            generated by hurricane impact assessment. If provided, the original
            failure probabilities will be saved as a separate worksheet.
    """
    ...
    prob_center_dict: Dict[int, np.ndarray] = {}  # 仅用于查找代表样本
    ...
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        ...
        # 保存原始故障概率数据（每个时刻的独立概率，不是累积概率）
        if linefail_path is not None and linefail_path.exists():
            try:
                linefail_xl = pd.ExcelFile(linefail_path)
                linefail_frames = []
                for sheet in linefail_xl.sheet_names:
                    df = pd.read_excel(linefail_xl, sheet_name=sheet)
                    df = _convert_linefailprob_sheet(df)
                    df.insert(0, "scenario_id", sheet)
                    linefail_frames.append(df)
                if linefail_frames:
                    original_linefail = pd.concat(linefail_frames, ignore_index=True)
                    original_linefail.to_excel(
                        writer, sheet_name="original_linefailprob", index=False
                    )
                    print(
                        "  原始故障概率已保存 ({} 行, {} 个场景，已转换为逐时独立概率)".format(
                            len(original_linefail), len(linefail_xl.sheet_names)
                        )
                    )
            except Exception as e:
                print(f"  警告: 无法加载原始故障概率: {e}")
...
def process_workbook(
    ...
    Args:
        linefail_path: Optional path to the original linefailprob.xlsx file.
            If provided, the original failure probabilities (from hurricane
            impact assessment) will be included in the output for reference.
    """
    ...
    print(
        "  聚类完成，结果已保存: {name} (工作表: cluster_assignments, "
        "cluster_summary, cluster_representatives, original_linefailprob)".format(
