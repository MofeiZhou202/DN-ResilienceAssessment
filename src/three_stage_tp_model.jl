...
function solve_fault_isolation(jpc, fault_lines)   
    ...
    println("线路开关状态 (β): ", β)
    println("VSC配置状态 (γ): ", γ)
    println("节点有功负荷削减 (pls): ", round.(pls, digits=2))
    println("线路有功功率 (pij): ", round.(pij, digits=2))
    println("发电机出力 (pg): ", round.(pg, digits=2))
    println("微电网发电机出力 (pm): ", round.(pm, digits=2))
    println("VSC交流传入直流功率 (prec): ", round.(prec, digits=2))
    println("VSC直流传入交流功率 (pinv): ", round.(pinv, digits=2))
...
function solve_post_fault_reconfig(jpc, stage1_result)
    ...
    println("节点有功负荷削减 (pls2): ", round.(pls2, digits=2))
    println("线路功率 (pij2): ", round.(pij2, digits=2))
    println("发电机出力 (pg2): ", round.(pg2, digits=2))
    println("微电网发电机出力 (pm2): ", round.(pm2, digits=2))
    println("VSC交流传入直流功率 (prec2): ", round.(prec2, digits=2))
    println("VSC直流传入交流功率 (pinv2): ", round.(pinv2, digits=2))
    println("节点有功负荷削减总和 (Pls2): ", round(Pls2, digits=2))
...
function solve_post_repair_reconfig(jpc, fault_lines, stage1_result, stage2_result)
    ...
    println("节点有功负荷削减 (pls3): ", round.(pls3, digits=2))
    println("线路功率 (pij3): ", round.(pij3, digits=2))
    println("发电机出力 (pg3): ", round.(pg3, digits=2))
    println("微电网发电机出力 (pm3): ", round.(pm3, digits=2))
    println("VSC交流传入直流功率 (prec3): ", round.(prec3, digits=2))
    println("VSC直流传入交流功率 (pinv3): ", round.(pinv3, digits=2))
    println("节点有功负荷削减 (Pls3): ", round(Pls3, digits=2))
